...
export class WardrobeMultiEditor {
  ...
  async handleImportMcaddon(e) {
    ...
    try {
      ...
      // 1. Read BP manifest
      const bpManifestFile = content.file(/.*(ZirconX-SKIN|ZXD44-SKIN)_BP\/manifest\.json$/i)[0] || content.file('ZirconX-SKIN_BP/manifest.json') || content.file('ZXD44-SKIN_BP/manifest.json');
      if (bpManifestFile) {
        try {
      ...
      const scriptFile = content.file(/.*(ZirconX-SKIN|ZXD44-SKIN)_BP\/scripts\/main\.js$/i)[0] || content.file('ZirconX-SKIN_BP/scripts/main.js') || content.file('ZXD44-SKIN_BP/scripts/main.js');
      if (scriptFile) {
...
`;
      ...
      const bpManifest = {
        ...
        header: {
          name: `ตู้เสื้อผ้า ZirconX ${this.addonVersion.join(".")}`,
      ...
      // Localization texts (Thai & English)
      const wardrobeLang = [
